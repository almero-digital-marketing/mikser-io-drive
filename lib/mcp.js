...
export function registerWebdavMcp({ runtime, base, endpoints, capabilityOf, writeCapabilityOf, logger }) {
    // MCP only, deliberately — not registered against the engine's registry.
    //
    // A CLI caller is already on the machine and already has the folders; a
    // WebDAV mount config is noise there. This answers "how do I, a REMOTE
    // client, reach these folders over HTTP", which is a question only a
    // remote caller has.
    //
    ...
    mcp.registerTool(
        // Prefixed, like every other tool this substrate registers — the bare
        // form belongs to the engine's registry, which this one stays out of.
        'mikser_webdav_config',
        {
            // Not mirrored into the engine's registry, so it never appears on
            // the CLI. A caller with a shell already has the folders.
            mcpOnly: true,
