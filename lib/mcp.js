// A door to the WebDAV endpoints this caller can reach, and the key to one.
//
...
// GET 200 with an OAuth token, 401 without — so a client with rclone or curl
// could always mount this. What it could not discover was WHICH endpoints
// exist, which of them its own capabilities allow it to write, and what the
// invocation looks like.
//
// MCP tools are the right shape for editing a page. They are the wrong shape
// for moving a gigabyte, and that is what this exists for: nothing transfers
// through the tool, so the bytes cost nothing in tokens.
//
// The credential it hands over is minted for the occasion — one endpoint,
// minutes not hours, revokable, never wider than the caller. It is NOT the
// caller's session bearer, which carries every endpoint for about an hour.
//
...
const rcloneBlock = (remote, url, bearer) =>
    `[${remote}]\ntype = webdav\nurl = ${url}\nvendor = other\nbearer_token = ${bearer}\n`
    ...
    mcp.registerTool(
        ...
        async ({ endpoint, write = false, ttl, allowContentWrite = false } = {}) => {
            ...
            return ok({
                ...
                renewable: false,
                // Runnable as written, with the real credential in them.
                //
                // These carried a $MIKSER_DAV_TOKEN placeholder so the secret
                // would appear exactly once. That property bought less than it
                // looked like — all the occurrences land in the SAME response,
                // so one copy leaking is one copy too many either way — and it
                // cost a substitution step nobody announced. An agent reading
                // the example either runs it literally and gets a 401, or has
                // to infer a step from a variable name. Both are worse than
                // the exposure being avoided.
                //
                // What actually bounds the risk is unchanged and is doing the
                // work: one endpoint, minutes not hours, revokable by jti,
                // never wider than the caller.
                examples: {
                    curlPut: `curl -H "Authorization: Bearer ${minted.token}" -T ./file.bin ${url}/file.bin`,
                    curlList: `curl -H "Authorization: Bearer ${minted.token}" -X PROPFIND -H 'Depth: 1' ${url}/`,
                    rclone: rcloneBlock(`mikser-${endpoint}`, url, minted.token),
                },
