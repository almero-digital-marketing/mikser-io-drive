    ...
    mcp.registerTool(
        'mikser_dav_token',
        {
            description:
                ...
                + `${TTL_DEFAULT}s, hard maximum ${TTL_MAX}s. Not renewable and not refreshable: expiry IS how it is `
                + 'revoked, so mint another if a transfer outlives one rather than asking for a longer window.\n\n'
            ...
            inputSchema: {
                ...
                ttl: { type: 'number',
                    description: `Lifetime in seconds. Default ${TTL_DEFAULT}, clamped to ${TTL_MAX}. The response says when it was clamped.` },
