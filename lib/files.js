...
//
// The extension table above is an allowlist, and an allowlist of file types is
// a list that goes stale silently: `.liquid` was refused as
// `application/octet-stream` — "not a viewable image or a text format" — for a
// template this very engine renders. Every other template extension was in the
// same position, and would have been fixed one bug report at a time as each
// render plugin shipped.
//
// Bytes do not go stale. A file with no NUL that decodes as UTF-8 is text,
// whether it is Liquid, Handlebars, TOML, CSV, an .env or something nobody has
// written yet. The extension still names the mime when it can; it just no
// longer gets a veto over whether the content comes back.
...
// Drop a trailing codepoint the sniff cut in half — and ONLY that.
//
...
// a JPEG whose last bytes happen to be bad reads as text. The tail is only
// forgiven when it is a valid multi-byte sequence that simply has not finished
// yet.
function trimPartialTail(buf) {
    ...
    for (let back = 1; back <= 4 && back <= buf.length; back++) {
        ...
        // `back` bytes are present from the lead to the end. Fewer than the
        // sequence needs means it was cut; anything else is complete or is
        // corrupt, and corrupt is the decoder's call to make, not ours.
