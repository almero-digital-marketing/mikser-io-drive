...
import { looksTextual, recordChangeSetWrite, checksumOf, explainRefusal, actingRole, useService, hasCapability, missingCapability } from 'mikser-io'
...
export function registerFileTools({ runtime, endpoints, capabilityOf, writeCapabilityOf, logger }) {
    // Asked of core rather than read off runtime.options.mcp, so this package
    // no longer reaches into a sibling's namespace and no longer depends on
    // where the mcp plugin sits in the plugins array.
    //
    // Registered through the SUBSTRATE, not the engine registry, on purpose:
    // these tools declare `mutates`, and the substrate is what gives a
    // mutating tool its changeSet parameters and runs its handler inside the
    // change set. Registering them against the engine would leave their
    // writes unattributable — the one failure nothing downstream can repair.
    // They still reach the CLI, because the substrate mirrors every
    // registration into the engine registry with the wrapped handler intact.
    ...
    // Both from the engine. Neither the "does this principal hold it" rule nor
    // the "which of these is missing" rule belongs to this plugin: a private
    // copy of the first is how the `*` wildcard came to work on the WebDAV
    // mount and not on the tools over the same endpoint.
    //
    // Writing needs the pair the mount has always required:
    //
    //   drive:<name>          may mount and read it
    //   drive:<name>:write    may ALSO write to it
    //
    // "also" means the base is a prerequisite, not an alternative. These tools
    // checked only the second, so a grant carrying `:write` without the base
    // was refused a PUT and allowed the identical write through here. That
    // configuration is not hypothetical — it is what a deployment has when its
    // editors hold `drive:documents:write` and nothing carries
    // `drive:documents`.
    const holds = (principal, capability) => hasCapability(principal, capability)
    const missingForWrite = (principal, endpoint) =>
        missingCapability(principal, [capabilityOf(endpoint), writeCapabilityOf(endpoint)])
    ...
    mcp.registerTool(
        ...
        async ({ endpoint, files, folder, overwrite = false, dryRun = false } = {}) => {
            ...
            const missingToAdd = missingForWrite(principal, endpoint)
            if (missingToAdd) {
                return fail(`Refused: you do not hold ${missingToAdd}. Nothing was written.`)
            }
            ...
            for (const s of staged) {
                ...
                recordChangeSetWrite({ uri: s.file, checksum: checksumOf(s.bytes) })
                written.push(s)
            }
    ...
    mcp.registerTool(
        ...
        async ({ paths, force = false, dryRun = false } = {}) => {
            ...
            for (const pathish of paths) {
                ...
                const missingToDelete = missingForWrite(principal, owning)
                if (missingToDelete) { refuse(`You do not hold ${missingToDelete}.`); continue }
                ...
                const referrers = reference && runtime.refs?.inboundFor ? runtime.refs.inboundFor(reference) : []
            ...
            const trashRoot = path.join(runtime.options.runtimeFolder ?? path.join(workingFolder(), 'runtime'),
                                        'trash', stamp)
            const results = []
            for (const s of staged) {
    ...
    mcp.registerTool(
        ...
        async ({ moves, rewriteRefs = false, dryRun = false } = {}) => {
            ...
            for (const [index, entry] of moves.entries()) {
                ...
                const missingToMove = missingForWrite(principal, owning)
                if (missingToMove) { refuse(`You do not hold ${missingToMove}.`); continue }
            ...
            for (const s of staged) {
                ...
                s.referrers = s.oldRef && runtime.refs?.inboundFor ? runtime.refs.inboundFor(s.oldRef) : []
                s.newRef = s.oldRef
