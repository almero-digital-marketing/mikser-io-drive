...
import { authorize, basicChallenge } from 'mikser-io'
// The one implementation of "does this principal hold it", including the
// `*` wildcard — checked here rather than re-derived, because two gates in
// this file and one in mikser-io-auth each had their own `.includes` and a
// wildcard added to the shared function reached none of them.
import { hasCapability } from 'mikser-io'
...
export class MikserAuthenticator {
    ...
    #writeCapability
    #realm
    // Whether the Basic challenge carries charset="UTF-8". Follows the
    // deployment's auth configuration rather than being decided here — see
    // basicChallenge in mikser-io.
    #charset
    #logger

    constructor({
        verifier, trustLoopback = false, allowRemote = false,
        capability, writeCapability, realm = 'mikser', charset = false, logger,
    } = {}) {
    ...
    async authenticate(request, response) {
        ...
        if (!outcome.ok) {
            ...
            if (outcome.status === 401) {
                response.set('WWW-Authenticate',
                    basicChallenge({ realm: this.#realm, charset: this.#charset }))
        ...
        if (this.#capability && !hasCapability(principal, this.#capability)) {
        ...
        if (this.#writeCapability && WRITE_METHODS.has(request.method) &&
            !hasCapability(principal, this.#writeCapability)) {
