...
export class MikserAuthenticator {
    ...
    async authenticate(request, response) {
        ...
        try {
            ...
            this.#logger?.error?.('webdav: verifier threw — %s', err.message)
        ...
        if (this.#capability && principal.capabilities != null &&
            ...
            this.#logger?.debug?.('webdav: %j lacks %j', principal.subject, this.#capability)
        ...
        if (this.#writeCapability && WRITE_METHODS.has(request.method) &&
            ...
            this.#logger?.debug?.('webdav: %j may not %s — lacks %j',
